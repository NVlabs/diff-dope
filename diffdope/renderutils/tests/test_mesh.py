...
def test_xfm_points():
	...
	ref_out = ru.xfm_points(points_ref.contiguous(), mtx_ref, use_python=True)
	ref_loss = torch.nn.MSELoss()(ref_out, target)
	ref_loss.backward()

	cuda_out = ru.xfm_points(points_cuda.contiguous(), mtx_cuda)
	cuda_loss = torch.nn.MSELoss()(cuda_out, target)
	cuda_loss.backward()

	print("---------------- CUDA ----------------------------")
	relative_loss("res:", ref_out, cuda_out)
	if points_ref.requires_grad:
		relative_loss("points:", points_ref.grad, points_cuda.grad)
	if mtx_ref.requires_grad:
		relative_loss("matrix:", mtx_ref.grad, mtx_cuda.grad)


def test_xfm_vectors():
	print("------Testing xfm_vector --------")
	...
	relative_loss("res:", ref_out, cuda_out)
	relative_loss("points:", points_ref.grad, points_cuda.grad)
	relative_loss("matrix:", mtx_ref.grad[:,:3,:3], mtx_cuda.grad[:,:3,:3])
