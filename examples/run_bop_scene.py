...
def main(cfg: DictConfig):
    ...
    for i_obj, obj in enumerate(frame_0):
        ...
        if not obj["obj_id"] in loaded_models:
            loaded_models[obj["obj_id"]] = dd.Mesh(
                f'{path_scene_bop_models}/obj_{str(obj["obj_id"]).zfill(6)}.ply',
                scale=0.01,
            )
        ...
        cv2.imwrite(
            f"{hydra.core.hydra_config.HydraConfig.get().runtime.output_dir}/{str(i_obj).zfill(2)}.png",
            img,
        )
